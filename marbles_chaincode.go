...
var totalTradedStr = "total_color"
...
func (t *SimpleChaincode) Init(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	err = stub.PutState(totalTradedStr, []byte("0"))
	if err != nil {
		return nil, err
	}
...
func (t *SimpleChaincode) set_user(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	var size int
	valAsbytes, err := stub.GetState(totalTradedStr)	//get the var from chaincode state

	var jsonResp string
	
	if err != nil {
		jsonResp = "{\"Error\":\"Failed to get state for " + totalTradedStr + "\"}"
		return nil, errors.New(jsonResp)
	}
	
	size , _ = strconv.Atoi(string(valAsbytes))
	...
	err = stub.PutState(totalTradedStr, []byte(s) )
	if err != nil {
		return nil, err
	}	
...
func (t *SimpleChaincode) perform_trade(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	fmt.Println("lenght",len(trades.OpenTrades))
